...
class TestToolDefinition:
    ...
    def test_adk_schema_no_input_schema(self):
    ...
    def test_adk_schema_with_input_schema(self):
        defn = self._make_defn(
            input_schema={
                "type": "object",
        ...
        params = defn.adk_schema["parameters"]
...
class TestToolTypeEntry:
    def test_fields(self, tmp_path):
        ...
        entry = ToolTypeEntry(
            name="api",
            config_schema_path=tmp_path / "api.yaml",
            handler_class=APIHandler,
        )
        assert entry.name == "api"
        assert entry.handler_class is APIHandler
        assert entry.config_schema_path.name == "api.yaml"
